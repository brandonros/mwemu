...
use super::registers::{
    read_regs_32, read_regs_64, read_regs_aarch64, write_regs_32, write_regs_64,
    write_regs_aarch64,
};
        ...
        match reg_id {
            AArch64RegId::X(n) => {
                ...
                let value = u64::from_le_bytes(
                    val[..8].try_into().map_err(|_| TargetError::NonFatal)?,
                );
                regs.x[n as usize] = value;
            }
            AArch64RegId::Sp => {
                regs.sp = u64::from_le_bytes(
                    val[..8].try_into().map_err(|_| TargetError::NonFatal)?,
                );
            }
            AArch64RegId::Pc => {
                regs.pc = u64::from_le_bytes(
                    val[..8].try_into().map_err(|_| TargetError::NonFatal)?,
                );
            }
            AArch64RegId::Pstate => {
                let value = u32::from_le_bytes(
                    val[..4].try_into().map_err(|_| TargetError::NonFatal)?,
                );
            ...
            AArch64RegId::V(n) => {
                ...
                regs.v[n as usize] = u128::from_le_bytes(
                    val[..16].try_into().map_err(|_| TargetError::NonFatal)?,
                );
            }
            _ => return Err(TargetError::NonFatal),
        }
